...
type Step =
  "aboutYou" | "pregnancyStart" | "pregnancyDetails" | "notificationPrivacy" | "journeyReady";
...
const PREGNANCY_FLAGS: PregnancyFlag[] = [
  "single",
  "twins",
  "ivf",
  "monitored",
...
export function OnboardingForm({ onSave }: OnboardingFormProps) {
  ...
  function errorsForStep(allErrors: Record<string, string>, s: (typeof STEP_ORDER)[number]) {
    return Object.fromEntries(
      Object.entries(allErrors).filter(([key]) => STEP_FIELDS[s].includes(key)),
    );
  }
  ...
  async function submit() {
    ...
    if (!result.ok) {
      ...
      const firstStepWithError = STEP_ORDER.find(
        (s) => Object.keys(errorsForStep(result.errors, s)).length > 0,
      );
  ...
  function toggleFlag(flag: PregnancyFlag) {
    ...
    const next = isSelected
      ? draft.pregnancyFlags.filter((f) => f !== flag)
      : [...draft.pregnancyFlags, flag];
    update({
      pregnancyFlags: next,
      sensitiveEverShown:
        draft.sensitiveEverShown || (!isSelected && SENSITIVE_FLAGS.includes(flag)),
    });
  }

  if (step === "journeyReady" && savedName) {
    return (
      <div className="mobile-screen flex min-h-dvh flex-col items-center bg-bg px-screen pt-[72px] pb-xl text-center">
        <div className="[&_.brand-rust]:[flood-color:var(--color-accent-primary)] [&_.brand-sage]:[flood-color:var(--color-accent-secondary)]">
          <BrandFilters />
          <Image
            src={journeyLogo}
            alt={t("start.logoAlt", { productName: PRODUCT_NAME })}
            width={590}
            height={773}
            sizes="120px"
            className="h-auto w-[120px] [filter:url(#brand-ink)]"
            preload
            unoptimized
          />
        </div>
        <h1 className="mt-xl font-display text-display text-text-primary">
          {t("onboarding.journeyReady.greeting", { name: savedName })}
        </h1>
        <p className="mt-sm max-w-[300px] text-body text-text-secondary">
  ...
  return (
    <div className="mobile-screen flex min-h-dvh flex-col gap-lg bg-bg px-screen py-xl">
      {step !== "aboutYou" && <BackButton label={t("common.back")} onClick={handleBack} />}
      {step === "aboutYou" && (
        <div className="flex flex-col gap-lg">
          ...
          />
          <Card surface="raised" className="flex flex-col gap-md">
            <h2 className="font-display text-h2 text-text-primary">
              {t("onboarding.aboutYou.emergencyTitle")}
            </h2>
            <Input
              id={emergencyNameId}
            ...
            <p className="text-caption text-text-secondary">
              {t("onboarding.aboutYou.emergencyNote")}
            </p>
          </Card>
        </div>
      )}
      ...
      {step === "pregnancyStart" && (
        <div className="flex flex-col gap-lg">
          <h1 id="pregnancy-start-title" className="font-display text-h1 text-text-primary">
            {t("onboarding.pregnancyStart.title")}
          </h1>
          <p className="text-body text-text-secondary">{t("onboarding.pregnancyStart.subtitle")}</p>
          <div role="radiogroup" aria-labelledby="pregnancy-start-title">
            <Card surface="raised" className="overflow-hidden p-0">
              {DUE_DATE_METHODS.map(({ method, hasDate }, index) => {
                const isSelected = draft.dueDateMethod === method;
                ...
                return (
                  <div key={method}>
                    <button
                      ...
                      onClick={() =>
                        update({ dueDateMethod: method, ...(hasDate ? {} : { date: "" }) })
                      }
                      className="tap-target flex w-full items-center justify-between gap-md p-[20px] text-left focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-accent-primary"
                    >
                      ...
                      <span
                        ...
                        className={`flex size-[22px] shrink-0 items-center justify-center rounded-full ${
                          isSelected
                            ...
                            : "border-[1.5px] border-divider-strong"
                        }`}
                      >
                        {isSelected && (
                          <span className="size-[11px] rounded-full bg-accent-primary" />
                        )}
                      </span>
                    </button>
                    {isSelected && hasDate && (
                    ...
                    {index < DUE_DATE_METHODS.length - 1 && (
                      <div className="mx-[20px] h-px bg-divider-strong" />
                    )}
                  </div>
                );
              })}
          ...
          {errors.dueDateMethod && (
            <p className="text-caption text-alert">{errors.dueDateMethod}</p>
          )}
        </div>
      )}
      ...
      {step === "pregnancyDetails" && (
        <div className="flex flex-col gap-lg">
          <h1 className="font-display text-h1 text-text-primary">
            {t("onboarding.pregnancyDetails.title")}
          </h1>
          <p className="text-body text-text-secondary">
            {t("onboarding.pregnancyDetails.subtitle")}
          </p>
          <div className="flex flex-wrap gap-sm">
            {PREGNANCY_FLAGS.map((flag) => (
              <button
                ...
                className="tap-target rounded-full border border-divider bg-surface-raised px-md py-sm text-body-sm text-text-primary data-[selected=true]:border-[1.5px] data-[selected=true]:border-peach data-[selected=true]:bg-peach data-[selected=true]:font-medium"
              >
          ...
          {draft.pregnancyFlags.includes("twins") && (
            <Card surface="raised" className="flex flex-col gap-sm">
              <p className="text-body text-text-primary">
                {t("onboarding.pregnancyDetails.twinQuestion")}
              </p>
              <div className="flex flex-wrap gap-sm">
          ...
          {draft.sensitiveEverShown && (
            <p className="text-body-sm text-text-secondary">
              {t("onboarding.pregnancyDetails.sensitiveNote")}
            </p>
          )}
        </div>
      )}
      ...
      {step === "notificationPrivacy" && (
        <div className="flex flex-col gap-lg">
          <h1 className="font-display text-h1 text-text-primary">
            {t("onboarding.notificationPrivacy.title")}
          </h1>
          <p className="text-body text-text-secondary">
            {t("onboarding.notificationPrivacy.intro")}
          </p>
          <div className="flex flex-col gap-sm">
            <Card
              interactive
              selected={draft.notificationPrivacy === "private"}
              selectedAccent="coral"
              surface="raised"
            ...
            >
              ...
              <p className="text-body-sm text-text-secondary">
                {t("onboarding.notificationPrivacy.privateDesc")}
              </p>
            </Card>
            <Card
              interactive
            ...
            >
              ...
              <p className="text-body-sm text-text-secondary">
                {t("onboarding.notificationPrivacy.detailedDesc")}
              </p>
            </Card>
          </div>
        </div>
