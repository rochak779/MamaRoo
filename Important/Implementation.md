...
> **Pre-deployment follow-up, updated 2026-09-12, resolved 2026-09-15:** the live Supabase project was using Supabase's default mailer, hard-capped at a couple of emails per hour — meant for testing only, not real OTP traffic. Custom SMTP is now configured (Resend, verified sending domain), the email rate limit has been raised (Supabase auto-bumped it to 30/hour on enabling custom SMTP), and a live `signInWithOtp` call against the production project returns 200 and delivers mail. **The "sign-in link" wording bug is fixed** — confirmed by inspecting both the "Confirm sign up" and "Magic link or OTP" Dashboard templates (both correctly render `Your code is {{ .Token }}`), by confirming no Auth Hook is configured to override them (Auth Hooks list is empty), and by pulling the actual delivered email content from the Resend dashboard for both a fresh test send and real prior sends (e.g. the 2026-09-14 17:13 send to a real user) — all show the plain numeric code (`Your code is 938705`, `Your code is 415330`), not a link. Root cause of the original bug was never pinned down (likely a stale send before the Dashboard template edit propagated), but the current state is confirmed correct end-to-end. OTP sign-in is safe to rely on for real users. No longer tracked as a blocker in Session 34's Gate C and release checklist.
...
> **Pre-deployment follow-up, added 2026-09-12, re-enabled 2026-09-15:** the consent gate was temporarily disabled at the product owner's request, so a signed-in user reached the onboarding form right after verifying her code, without a consent screen in between. `CONSENT_REQUIRED` in `lib/domain/routing.ts` is flipped back to `true` ahead of launch, with `routing.test.ts` updated to match (an unconsented authed user now lands on `/consent` again, and staying on `/consent` is a no-op instead of skipping to onboarding). **The legal drafts are still explicitly marked "DRAFT, NOT LEGALLY REVIEWED"** in all four `content/legal/*.md` files — that has not been resolved, and the product owner made an informed call to launch with the draft text rather than hold for counsel review. Get real legal review post-launch and update those files (no code change needed beyond swapping the markdown, since `LEGAL_VERSION` in `lib/config.ts` already exists to version-stamp consent rows against policy text changes). Also as part of this pass: `ConsentForm` was restyled onto the shared `.auth-screen`/`.auth-title` shell (`styles/start.css`) so it stops visually breaking stride between the redesigned Verify Code and Onboarding Profile screens either side of it — it had never been touched by the Sessions 45-53 redesign-alignment project. And the redundant language switcher was removed from `LegalDocument` (Privacy/Terms): those pages now just render in whatever locale `/welcome` (or the consent screen's own switcher) already set, rather than offering a second switcher one step later for everyone who reaches them normally.
